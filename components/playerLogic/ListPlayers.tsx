	...
	return (
		<div className='border border-x-0 border-t-0 py-2 flex flex-wrap items-center'>
			<div className='flex flex-col justify-center items-center mr-3'>
				...
				<Link href='/'>
					<button title='Add more players' className={`${styles.foreground} ${styles.card} hover:text-green-500 m-1 py-2 px-3 cursor-pointer`}>
						<i className="bi bi-person-plus-fill"></i>
					</button>
				</Link>
			</div>
			{players && players.map((player, idx) => (
				<div key={idx} className={`${styles.foreground} rounded shadow-sm m-1 p-2 relative border border-zinc-300 dark:border-zinc-600`}>
